...
import {useCreateNewsMutation, useGetAllNewsMutation,} from "../../store/auth/authApiSlice";
...
export default function News() {
    ...
    const [newNews, setNewNews] = useState({
        name: "", publishDate: "", desc: "", emailUser: email
    });
    ...
    return (
        <>
            <main>
                ...
                <div>
                    {newsReverse.slice(0, 1).map((post) => (
                        <MainFeaturedPost key={post.name} post={post}/>
                    ))}
                    <div className="template-grid">
                        {newsReverse.slice(1).map((post) => (
