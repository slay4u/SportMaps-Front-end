...
import { Avatar, Button, Container, Divider, Typography, Grid } from '@mui/material';
import TurnedInNotIcon from '@mui/icons-material/TurnedInNot';
import { useNavigate } from 'react-router-dom';
import store from '../../store/store';
import { useNewsCommentMutation, useGetNewsByIdMutation, useGetUserByEmailMutation, useDeleteNewsMutation, useDeleteNewsCommentMutation, useUpdateNewsMutation } from '../../store/auth/authApiSlice';
...
export default function NewsPage() {
  ...
  const editNews = () => {
    updateNews.publishDate = new Date().toLocaleDateString('uk-UA') + " " + new Date().toLocaleTimeString('uk-UA').slice(0,5);
  ...
  const submitComment = () => {
    comment.createdDate = new Date().toLocaleDateString('uk-UA') + " " + new Date().toLocaleTimeString('uk-UA').slice(0,5);
    commentCall(comment);
    window.location.reload();
  }
        ...
        <Container maxWidth="xl" sx={{minHeight: '970px'}}>
          <main>
            <img src="https://source.unsplash.com/random/?sport/" alt="sport-img"
              style={{objectFit: 'cover', width: '100%', height: '750px', filter: "brightness(60%)"}}>
            </img>
            <Box sx={{ display: "flex"}}>
              <Typography
                variant="h3"
              ...
              <div style={{paddingTop:"1em", display:'flex', justifyContent: 'center'}}>
                <div className="popup" id="popup">
                  ...
                  <button className="createNewNewsButton" type="submit" onClick={editNews}>Редагувати</button>
                </div>
              </div>

              <div style={{marginTop: '5em', marginLeft: '77em', position: 'absolute'}}>
                <button className="createNewNewsButton" style={{backgroundColor: 'blue', borderColor: 'blue'}} onClick={openPopup} >Редагувати</button>
              </div>
              <div style={{marginTop: '5em', marginLeft: '86em', position: 'absolute'}}>
                <button className="createNewNewsButton" style={{backgroundColor: 'red', borderColor: 'red'}} onClick={deleteNews}>Видалити</button>
              </div>
            </Box>
            <Box sx={{ display: "flex", pl: "5%"}}>
              ...
              <Box sx={{ ml: "1em"}}>
                ...
                <Divider />
                <Typography>
                  {new Date(news.publishDate).toLocaleDateString() + " " + new Date(news.publishDate).toLocaleTimeString().slice(0,5)}
                </Typography>
              </Box>
            </Box>
            <Divider sx={{mt: "1em", backgroundColor: 'black'}}/>
            <Box sx={{pt: "2em", pl:"1em", pr:"0.7em"}}>
              <Typography variant="subtitle1" paragraph sx={{fontSize: '25px'}}>
                {news.desc}
              </Typography>
            </Box>
            ...
            <div style={{width: "50%", paddingLeft: '1em'}}>
              ...
              <textarea
                id='newsCommentArea'
                style={{ minWidth: "40em", minHeight: "5em", maxWidth: "60em", maxHeight: "15em", borderRadius: "0.25em"}}
                onChange={(e) => updateComment(e.target.value)}
              ></textarea>
              <div style={{marginTop: '0.3em'}}>
                <button id="newsCommentsSubmit" type="submit" onClick={submitComment}
                >
                  Надіслати
                </button>
              </div>
              <Grid container spacing={2} columns={1} sx={{ mt: "1em", pb: '1em', width: "100%"}}>
                {news.commentList && news.commentList.map((comment) => (
